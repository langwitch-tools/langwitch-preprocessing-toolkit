...
fn main() {
    ...
    tool! {
        ...
        body: || {
            ...
            let lengths = get_lengths(&desired_lang, &reference_files);
...
fn get_lengths(desired_lang: &str, language_dir: &str) -> HashMap<String, (memmap::Mmap, usize)> {
    let files = get_files(language_dir)
        .unwrap()
        .into_iter()
        .enumerate()
        .filter(|(i, c)| i % 15 == 0 || c.to_string_lossy().to_string() == desired_lang)
        .map(|(_i, c)| c)
